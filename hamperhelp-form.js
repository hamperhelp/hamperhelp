...
function activatePickupDay(day) {
  ...
  document.querySelectorAll('#pickup-day-buttons button')
    .forEach(b => b.classList.toggle('selected', b.id === `btn-${day}`));
  ...
  // 🆕 show helper + calendar ONLY for "later"
  const isLater = (day === 'later');
  document.getElementById('later-help').style.display  = isLater ? 'block' : 'none';
  document.getElementById('later-date').style.display  = isLater ? 'inline-block' : 'none';
}
...
// NEW:
document.getElementById("btn-later").addEventListener("click", () => {
  activatePickupDay("later");        // highlight, helper, calendar
  renderPickupWindows("later");      // (optional) prefill slots
});
