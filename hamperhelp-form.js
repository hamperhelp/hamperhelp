...
function addLoad() {
  ...
  selectedLoads.push({ type: null, rush: false });
  loadCount++;
  updateLoadButtons();
  updateOrderTotal();
}
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const laterDate = document.getElementById("later-date");
  if (laterDate) {
    const today = new Date();
    const minDate = today.toISOString().split("T")[0];
    const maxDate = new Date(today.setDate(today.getDate() + 6)).toISOString().split("T")[0];
    laterDate.min = minDate;
    laterDate.max = maxDate;
  }

  const autocompleteInput = document.getElementById("address");
  if (autocompleteInput && window.google && google.maps) {
