...
function checkPostalCode() {
  ...
  if (postalCodes.includes(cleaned)) {
    ...
    renderPickupWindows("today");
    renderPickupWindows("tomorrow");
  } else {
...
function renderPickupWindows(type) {
  ...
  document.querySelector(`.pickup-btn-group[data-type="${type}"]`)?.classList.add("active");
}
...
function updateOrderTotal() {
  totalPrice = 0;
  document.querySelectorAll(".load-entry").forEach((entry, i) => {
    const scent = entry.querySelector("select:nth-of-type(3)").value;
