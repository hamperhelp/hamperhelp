...
function selectOption(index, type, btn) {
...
function selectRush(index, rush, btn) {
...
function updateOrderTotal() {
  totalPrice = 0;
  const loads = document.querySelectorAll(".load-entry");
  loads.forEach((el, i) => {
    const scent = el.querySelector("select:nth-of-type(3)").value;
    selectedLoads[i].scent = scent;
    if (selectedLoads[i].type === 'basket') totalPrice += 30;
    if (selectedLoads[i].type === 'hamper') totalPrice += 50;
    if (selectedLoads[i].rush) totalPrice += 10;
    if (scent !== 'None') totalPrice += 0.75;
  });
