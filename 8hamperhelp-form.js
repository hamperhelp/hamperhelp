// ===========================
// HamperHelp Form Logic JS (Full Logic Included, Structured)
// ===========================
...
function renderPickupWindows(type) {
  ...
  windows.forEach(w => {
    let text = w.label;
    const minsLeft = (w.start * 60) - (hours * 60 + mins);
  ...
  document.getElementById("time-today").classList.remove("active");
  document.getElementById("time-tomorrow").classList.remove("active");
  document.getElementById("time-later").classList.remove("active");
  document.getElementById(`time-${type}`).innerHTML = html;
  document.getElementById(`time-${type}`).classList.add("active");
}
...
function addLoad() {
  ...
  load.innerHTML = `
    ...
    <div>
      <button class="btn-basket" id="basket-${id}" onclick="selectOption(${id}, 'basket')">Basket — up to 16 lbs — $30</button>
      <div style="text-align: center; margin: 5px 0;">or</div>
      <button class="btn-hamper" id="hamper-${id}" onclick="selectOption(${id}, 'hamper')">Hamper — up to 32 lbs — $50</button>
    </div>
    <h5>Need it in 12 hours?</h5>
    <div>
      <button class="rush-yes" id="rush-yes-${id}" onclick="selectRush(${id}, true)">Yes, I’m in a hurry +$10</button>
      <div style="text-align: center; margin: 5px 0;">or</div>
      <button class="rush-no" id="rush-no-${id}" onclick="selectRush(${id}, false)">No thanks — 24 hours is fine</button>
    </div>
    <h5>Free Options</h5>
    ...
    <label>Just 75¢ for scent that turns heads — choose your aroma:
      <select class="scent-select">
        <option>None</option><option>Gain Original</option><option>Island Fresh</option>
      </select>
    </label>
  `;
...
function selectOption(index, type) {
  ...
  document.getElementById(`basket-${index}`).style.background = type === 'basket' ? '#007bff' : '';
  document.getElementById(`hamper-${index}`).style.background = type === 'hamper' ? '#007bff' : '';
  updateOrderTotal();
}

function selectRush(index, rush) {
  selectedLoads[index].rush = rush;
  document.getElementById(`rush-yes-${index}`).style.background = rush ? '#007bff' : '';
  document.getElementById(`rush-no-${index}`).style.background = !rush ? '#007bff' : '';
  updateOrderTotal();
}

function setupScentListeners() {
  document.querySelectorAll('.scent-select').forEach((el, i) => {
    el.addEventListener('change', () => {
      selectedLoads[i].scent = el.value;
      updateOrderTotal();
    });
  });
}
...
function updateOrderTotal() {
  ...
  selectedLoads.forEach(load => {
    ...
    if (load.scent && load.scent !== 'None') totalPrice += 0.75;
  });
